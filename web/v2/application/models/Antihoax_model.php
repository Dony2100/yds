...
class Antihoax_model extends CI_Model
{
    ...
    public function get_all($limit = 10, $offset = 0, $random = false)
    {
        ...
        $tags = $this->input->get('tags');
        if($tags !== NULL){
            $tags = " WHERE a.tags LIKE '%cekfakta%' ";
        }else{
            $tags = "";
        }
    ...
    {
        $sql = "select a.id, a.nama, a.deskripsi, a.website, concat('https://yudistira.turnbackhoax.id/uploads/thumbnail/', a.icon) as icon from author a";
    ...
    {
        $sql = "select a.id, a.nama, a.deskripsi, a.website, concat('https://yudistira.turnbackhoax.id/uploads/thumbnail/', a.icon) as icon from author a where a.id = ?";
        $query = $this->db->query($sql, array($id));
        return $query->row();
    }
