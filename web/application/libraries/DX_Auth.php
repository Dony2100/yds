...
*/
    ...
    function check_uri_permissions($allow = TRUE, $req_role = '') {
        ...
        if ($this->is_logged_in()) {
            ...
            if (!$this->is_admin()) {
                ...
                //** addition Andy, check permission as requested
                if($req_role && $this->check_permission_by_text($req_role)) $have_access = $allow;
    ...
    function check_permission_by_text($role_request) {
        $role_name = $this->ci->session->userdata('DX_role_name');
        if(is_array($role_request))
            if(in_array($role_name, $role_request)) return TRUE;
        else    
            if($role_request == $role_name) return TRUE;
    }
    ...
    function register($username, $password, $email) {
        ...
        $new_user = array(
            'username' => $username,
            'password' => crypt($this->_encode($password), password_hash(strrev($email),PASSWORD_DEFAULT,["cost" => 9])),
            'email' => $email,
    ...
    function forced_chg_passwd($id, $email, $pass) {
        // Load Models
        $this->ci->load->model('dx_auth/users', 'users');
        
        // Crypt and encode new password
        $new_pass = crypt($this->_encode($pass), password_hash(strrev($email),PASSWORD_DEFAULT,["cost" => 9]));

        // Replace old password with new password
        return $this->ci->users->change_password($id, $new_pass);
    }
    ...
    function kocok($keyword, $level = 11) {
        $majorsalt = $this->ci->config->item('DX_salt');
        $xsalt = hash('tiger128,3', $majorsalt);
        
        $prep = hash_init("haval256,5");
        hash_update($prep, $this->_encode($keyword));
        hash_update($prep, crypt($xsalt, '$5$rounds=5678$' . strrev($majorsalt) . '$'));
        $hash = hash_final($prep);

        $res = substr(sha1(substr($hash, 32)), -39, -9);        
        $ult = substr(sha1(substr($hash, 0, 32)), -33, -3);
        return substr($res,0,8).strrev(substr($res.$ult,8,35)).substr($ult, -17);
    }
    
    function add_user($username, $password, $email, $other) {
    ...
    function get_roles($role_id, $field=FALSE) {
        $this->ci->load->model('dx_auth/roles', 'role');
        $query = $this->ci->role->get_role_by_id($role_id); //var_dump($result);
        
        if ($query->num_rows() > 0) {
            // Get row
            $role = $query->row();
            if($field) return $role->name;
            else return $role;
        }
    }
