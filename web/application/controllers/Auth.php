...
class Auth extends CI_Controller
{
	...
	function login()
	{
		if ( ! $this->dx_auth->is_logged_in())
		{
			...
			{
				$val->set_rules('recaptcha_response_field', 'Confirmation Code', 'trim|required|callback_captcha_check');
			}
			...
			else
			{
				...
				{						
					...
					$data['_view'] = $this->dx_auth->login_view;
                                        $data['_head'] = "_login";
		...
		else
		{
			...
			$data['_view'] = $this->dx_auth->logged_in_view;
                        $data['_head'] = "_message";
	...
	function logout()
	{
		...
		$data['_view'] = $this->dx_auth->logout_view;
                $data['_head'] = "_message";
		$this->load->view('layouts/main',$data);
	}
	...
	function register()
	{
		...
		{	
			...
			$val->set_rules('password', '', 'trim|required|min_length['.$this->min_password.']|max_length['.$this->max_password.']|matches[confirm_password]');
			$val->set_rules('confirm_', 'Confirm Password', 'trim|required');
	...
	function forgot_password()
	{
		...
		else
		{
                    ...
                    $data['_view']=$this->dx_auth->forgot_password_view;
                    $data['_head']="_login";
                    $this->load->view("layouts/main",$data);
		}
	}
	...
	function change_password()
	{
		...
		{			
			...
			// Set form validation
			$val->set_rules('old_password', 'Old Password', 'trim|required');
			...
			else
			{
                            ...
                            $data['_view']=$this->dx_auth->change_password_view;
                            $data['_head']="_login";
                            $this->load->view("layouts/main",$data);
			}
		}
	...
	function custom_permissions()
	{
     ...
     function deny() {
        $data['auth_message'] = 'Your don\'t have permission to access this page.';
        $data['_view']="Auth/general_message";
        $data['_head']="_message";
        $this->load->view("layouts/main",$data);        
     }
     
     function banned() {
        $data['auth_message'] = 'Unauthorized: Your account has been suspended.';
        $data['_view']="Auth/general_message";
        $data['_head']="_message";
        $this->load->view("layouts/main",$data);           
     }
